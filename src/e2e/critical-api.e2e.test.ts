...
describe("phase 4.5 critical API-first e2e", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("critical #2: denies access when unauthenticated or with insufficient role", async () => {
    withAuthMock.mockRejectedValueOnce(new AppError("Not authenticated", "UNAUTHORIZED", 401));

    const unauthenticatedResponse = await getDocumentRoute(
      new NextRequest("http://localhost/api/encounters/101/documents/MEDICAL_CERTIFICATE"),
      {
        params: Promise.resolve({ encounterId: "101", documentTypeCode: "MEDICAL_CERTIFICATE" }),
      }
    ...
    expect(unauthenticatedBody).toEqual({ error: "Not authenticated" });

    withAuthMock.mockRejectedValueOnce(new ForbiddenError("Insufficient permissions"));

    const forbiddenResponse = await getDocumentRoute(
      new NextRequest("http://localhost/api/encounters/101/documents/MEDICAL_CERTIFICATE"),
      {
        params: Promise.resolve({ encounterId: "101", documentTypeCode: "MEDICAL_CERTIFICATE" }),
      }
    ...
    expect(forbiddenBody).toEqual({ error: "Insufficient permissions" });
  });

  it("critical #3: returns 400 and fieldErrors for invalid encounter payload", async () => {
    ...
    const httpLike = {
      status: actionResponse.success ? 200 : 400,
      body: actionResponse,
    };

    expect(httpLike.status).toBe(400);
    expect(httpLike.body.success).toBe(false);
    if (httpLike.body.success) return;
    expect(httpLike.body.fieldErrors).toHaveProperty("diagnoses");
  });
});
